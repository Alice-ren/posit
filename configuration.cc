...
configuration::configuration(double prob = 0.0, double quality = 0.0) {
  this->prob = prob;
...
void configuration::print(unsigned tab_layers = 0) const {
  if(expanded())
    cout << "-->";
  else
    cout << "   ";
...
void configuration::enumerate_subconfigs(const list<configuration> &subconfigs) const {
  if(expanded()) {
...
configuration configuration::highest_quality_subconfig(configuration current_best = configuration()) const {
